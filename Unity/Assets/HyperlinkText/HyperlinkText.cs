...
/// 文本控件,支持超链接
/// </summary>
[AddComponentMenu("UI/HyperlinkText", 10)]
public class HyperlinkText : Text, IPointerClickHandler
{
    ...
    const string colorStartStr = "<color=blue>";
    ...
    //     {
    //         List<RectToDrawInfo> infos = new List<RectToDrawInfo>();

    //         foreach (var hrefInfo in hrefInfos)
    //         {
    //             foreach (var area in hrefInfo.textValidAreas)
    //             {
    //                 //box里面信息是Text在局部坐标系的
    //                 //需要转换到世界坐标系（UI的世界坐标系是屏幕坐标系）
    //                 Vector2 screenPosMin = transform.TransformPoint(area.min);
    //                 Vector2 screenPosMax = transform.TransformPoint(area.max);

    //                 RectToDrawInfo rectToDrawInfo =
    //                     new RectToDrawInfo(screenPosMin, screenPosMax);
    //                 infos.Add(rectToDrawInfo);
    //             }

    //         }

    //         DrawRect.Instance.SetDrawInfo(infos);
    //     }
